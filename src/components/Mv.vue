    ...
    <ul>
      <li v-for="(item, index) in mvData" :key="index">
        <div class='show'>
          ...
          <div class="count"><i class="el-icon-view"></i>{{item.playCount.toLocaleString()}}</div>
          <div class="time">{{timeShow(item.duration / 1000)}}</div>
        </div>
        <div class="name"><a href="javascript:;" :title="item.name">{{item.name}}</a></div>
        <div class="aughor"><el-link type="info">{{item.artistName}}</el-link></div>
      </li>
    </ul>
  </div>
</template>
